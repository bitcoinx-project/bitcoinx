...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        if (scriptPubKey.size() > ScriptConf::MAX_SCRIPT_ELEMENT_SIZE()) {
            throw std::runtime_error(strprintf(
                        "redeemScript exceeds size limit: %d > %d", scriptPubKey.size(), ScriptConf::MAX_SCRIPT_ELEMENT_SIZE()));
        }
        // Get the address for the redeem script, then call
        // GetScriptForDestination() to construct a P2SH scriptPubKey.
...
static void MutateTxAddOutScript(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (scriptPubKey.size() > ScriptConf::MAX_SCRIPT_SIZE()) {
        throw std::runtime_error(strprintf(
                    "script exceeds size limit: %d > %d", scriptPubKey.size(), ScriptConf::MAX_SCRIPT_SIZE()));
    }
