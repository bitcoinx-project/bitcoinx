...
void checkResult(bool isCreation, std::vector<EthTransaction> results, uint256 hash){
    for(size_t i = 0; i < results.size(); i++){
        ...
        BOOST_CHECK(results[i].value() == value * SATOSHI_2_WEI_RATE);
        ...
        BOOST_CHECK(results[i].GetOutIdx() == i);
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    EthTxConverter converter(transaction, NULL);
    std::vector<EthTransaction> ethTx;
    BOOST_CHECK(converter.Convert(ethTx));
    std::vector<EthTransaction> result = ethTx;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ethtxconverter_tests, TestingSetup)

BOOST_AUTO_TEST_CASE(parse_txcreate){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << OP_CREATECONTRACT;
    runTest(true, 1, script1);
}

BOOST_AUTO_TEST_CASE(parse_txcall){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << OP_SENDTOCONTRACT;
...
BOOST_AUTO_TEST_CASE(parse_incorrect_txcreate_many){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << OP_CREATECONTRACT;
    CScript script2 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << OP_CREATECONTRACT;
    runFailingTest(true, 120, script1, script2);
}

BOOST_AUTO_TEST_CASE(parse_incorrect_txcreate_few){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << OP_CREATECONTRACT;
    CScript script2 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << OP_CREATECONTRACT;

    runFailingTest(true, 4, script1, script2);
}

BOOST_AUTO_TEST_CASE(parse_incorrect_txcall_many){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << OP_SENDTOCONTRACT;
    CScript script2 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << address << OP_SENDTOCONTRACT;
    runFailingTest(false, 120, script1, script2);
}

BOOST_AUTO_TEST_CASE(parse_incorrect_txcall_few){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << OP_SENDTOCONTRACT;
    CScript script2 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << OP_SENDTOCONTRACT;
    runFailingTest(false, 120, script1, script2);
}

BOOST_AUTO_TEST_CASE(parse_incorrect_txcall_overflow){
    CScript script1 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(gasLimit)) << CScriptNum(int64_t(gasPrice)) << data << address << OP_SENDTOCONTRACT;
    CScript script2 = CScript() << CScriptNum(EthTxVersion::GetDefault().ToRaw()) << CScriptNum(int64_t(0x80841e0000000000)) << CScriptNum(int64_t(0x0100000000000000)) << data << address << OP_SENDTOCONTRACT;
    runFailingTest(false, 120, script1, script2);
}
