...
bool WalletModel::isUnspentAddress(const std::string &BitcoinXAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    for (const COutput& out : vecOutputs)
    {
        ...
        if(fValidAddress && CBitcoinAddress(address).ToString() == BitcoinXAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
