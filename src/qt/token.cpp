...
bool ToBitcoinXAddress(const std::string& strHash160, std::string& strBitcoinXAddress)
{
...
bool ToHash160(const std::string& strBitcoinXAddress, std::string& strHash160)
{
    CBitcoinAddress BitcoinXAddress(strBitcoinXAddress);
    if(BitcoinXAddress.IsValid()){
        CKeyID keyid;
        BitcoinXAddress.GetKeyID(keyid);
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBitcoinXAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBitcoinXAddress(tokenEvent.receiver, tokenEvent.receiver);
